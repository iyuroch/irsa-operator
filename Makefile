...
# irsa.aws/irsa-operator-bundle:$VERSION and irsa.aws/irsa-operator-catalog:$VERSION.
IMAGE_TAG_BASE ?= irsa.aws/irsa-operator
...
# 	export KO_DOCKER_REPO= && ko build
...
deploy: manifests kustomize image ## Deploy controller to the K8s cluster specified in ~/.kube/config.
